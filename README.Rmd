...
The whole graph contains `r vcount(ig)` nodes (journals).
Of these, `r strong$csize[core]` belong to our "core" strongly-connected component and `r sum(strong$csize == 1)` are singletons, either weakly connected or completely disconnected from the rest of the graph.
...
We now have `r vcount(ig)` journals in our network.
...
```{r superjournals, dev = 'pdf', fig.width = 10, fig.height = 10}
...
```{r labelled, dev = 'png', fig.width = 10, fig.height = 10, dev.args = list(type = 'cairo')}
extrafont::loadfonts(device = 'win', quiet = TRUE)

ggraph(my_mds) +
  geom_edge_fan0(alpha = .01, colour = 'steelblue3') +
  geom_node_point(aes(size = PageRank), fill = 'steelblue3', pch = 21, colour = 'white') +
  geom_node_label(aes(label = field), repel = TRUE, size = 1, family = 'Arial Narrow') +
  coord_fixed() +
  scale_x_reverse() +
  theme_graph() +
  theme(legend.position = 'none')
